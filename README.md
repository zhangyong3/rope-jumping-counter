...
检测器使用肩、髋、膝和脚踝关键点。稳定站立后才建立基线，脚踝与髋部的垂直位移使用躯干长度归一化；连续跳绳时，每一跳都以最近一次落地低点作为新的局部基准，避免人体关键点缓慢漂移导致计数中断。信号经过指数平滑后进入 `READY → RISING → AIRBORNE → LANDING` 状态机，只有双脚同步上升、人体尺度稳定，并满足相对峰值高度、持续时间和最短间隔的完整周期才会计数。
